from flask import Flask, render_template, redirect, url_for, flash, request, session, get_flashed_messages, Response, jsonify
...
app = Flask(__name__)
user_tips = []
user_data = pd.DataFrame(columns=['user_id', 'recipe_id', 'rating'])
...
def surprise_me():
    ...
    recipe = db.session.query(Recipe).order_by(db.func.random()).first()
    return render_template('surprise_me.html', recipe=recipe)
...
def export_shopping_list():
    """Exports the user's shopping list as a CSV file."""
    ...
    # Query the database for user's meal plans and recipes
    ...
    # Write each ingredient as a new row in the CSV
    for item in items:
...
@app.route('/view/<int:mealplan_id>')
def view_mealplan(mealplan_id):
    mealplan = MealPlan.query.get_or_404(mealplan_id)
    return render_template('view_mealplan.html', mealplan=mealplan)

@app.route('/edit/<int:mealplan_id>', methods=['GET', 'POST'])
def edit_mealplan(mealplan_id):
    mealplan = MealPlan.query.get_or_404(mealplan_id)

    if request.method == 'POST':
        mealplan.name = request.form['name']
        mealplan.date = request.form['date']
        db.session.commit()
        flash('Meal Plan updated successfully', 'success')
        return redirect(url_for('dashboard'))

    return render_template('edit_mealplan.html', mealplan=mealplan)

@app.route('/delete/<int:mealplan_id>', methods=['POST'])
...
def add_tip():
    if request.method == 'POST':
        # Get the user's submitted tip
        new_tip = request.form.get('new_tip')
        
        # If the tip is not empty, add it to the list
        if new_tip:
            user_tips.append(new_tip)
            flash("Your tip has been added successfully!", "success")
        else:
            flash("Please enter a tip before submitting.", "danger")
        
        return redirect(url_for('add_tip'))  # Redirect to the same page after submission

    # Render the template for displaying tips and submitting new ones
    return render_template('add_tip.html', user_tips=user_tips)
...
@login_required
def budget_summary():
    # Example user-defined budget (can be made dynamic later)
    weekly_budget = 100.0
    ...
    over_budget = total_cost > weekly_budget

    return render_template('budget_summary.html', recipes=weekly_recipes,total_cost=total_cost, weekly_budget=weekly_budget, over_budget=over_budget)
